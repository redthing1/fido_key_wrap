...
- generating or importing the random root and generating separate binary factors
...
- choosing and operating trusted storage for each local secret
...
the nine policies are building blocks. an application may expose one, several,
or all of them.
...
| fido presence plus local secret | credential, touch, and generated local secret | supports touch-only daily use on a paired machine |
...
## macos user-presence recovery

the platform crate can store a generated recovery secret in the macos
data-protection keychain and require local user authentication accepted by
macos whenever it is read:
...
this uses the existing `RecoverySecret` recipient. the envelope stays portable,
but the stored factor is local, non-synchronizing, and subject to the caller's
keychain access group and code-signing controls. an application that needs
export, recovery on another machine, or another platform supplies a separate
recovery route or storage policy.

store the factor before publishing a container that references it. to remove
it, load the secret, authenticate the exact envelope, then call
`store.remove(recipient, &secret)`. see the
[platform storage guide](platform-storage.md) for the complete operation
contract.

## paired-machine recovery

`FidoPresenceAndLocalSecret` combines a dedicated non-discoverable security-key
credential with a fresh 32-byte secret held by the application. enrollment
requires the authenticator pin and a touch. later recovery requires the local
secret and a touch, with no pin or application passphrase.

create the route through its dedicated method:

```rust,ignore
use fido_key_wrap_platform::LocalSecretStore;

let (root, envelope, local) = protector
    .create_root_with_fido_and_local_secret("paired machine", &mut interaction)?;

let secrets = fido_key_wrap_platform::NativeLocalSecretStore::new(
    trusted_application_id.clone(),
);
secrets.create(local.recipient_id(), local.secret())?;
```

load the exact secret and select the returned recipient when opening:

```rust,ignore
let recipient = selected_recipient_id;
let secret = secrets.load(recipient)?;
let root = protector.unlock_with_fido_and_local_secret(
    &envelope,
    recipient,
    &secret,
    &mut interaction,
...
`protect_root_with_fido_and_local_secret` protects an existing random root, and
`add_fido_and_local_secret` adds the route transactionally to an authenticated
envelope. the generic `Enrollment` and `unlock` methods do not accept this
policy, which prevents either factor from being omitted accidentally.

the local secret is not present in the envelope. the optional
`fido-key-wrap-platform` crate binds it to the trusted application and recipient
identity in the native user secret store. applications can instead use an
existing store with the same binding. loss of either the local secret or
security-key credential makes the route unavailable. copying the local secret
to another machine extends the pairing to that machine.

when creating or adding this route, durably store the local secret under the
trusted application id and returned recipient id before publishing the
container that references it. if container publication is uncertain, retain
the secret and reconcile the stored state; if publication definitely fails,
delete the unpublished entry. when removing the route, publish the replacement
container before deleting its local secret.

this route uses a non-discoverable credential and consumes no discoverable
credential slot. it does not provide managed retirement or cryptographic
erasure of retained envelopes.

native storage can block or prompt. call it on a bounded blocking worker, not
directly on an executor or user interface thread. see the
[platform storage guide](platform-storage.md) for backend behavior and
publication order.
...
4. pass that exact `RecipientId` to `unlock`,
   `unlock_with_recovery_secret`, or
   `unlock_with_fido_and_local_secret`, according to the selected policy
...
`add_recipient`, `add_recovery_secret`, `add_fido_and_local_secret`,
`remove_recipient`, and `rewrap_passphrase` require the current root and
authenticate the current envelope before returning. they stage a complete
replacement and leave the caller's envelope unchanged on failure.
